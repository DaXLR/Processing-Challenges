//Variables pour la grosseur de l'ecran (overwritten par fullscreen si actif)
...
{
  coord result = new coord(0, 0, 0);
  float screenSpaceX = tan(abs(cartesianYaw))*cam.focalLength;
  float screenSpaceY = tan(abs(cartesianPitch))*cam.focalLength;
